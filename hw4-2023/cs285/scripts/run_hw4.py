...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        for i in range(mb_agent.ensemble_size):
            next_ob_preds.append(mb_agent.get_dynamics_predictions(i, ob[None], ac[None]))
        next_ob = np.mean(next_ob_preds, axis=0)[0]
        
        rew, _ = env.get_reward(next_ob[None], ac[None])
        rew = rew[0]
